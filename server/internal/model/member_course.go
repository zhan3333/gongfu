...
type MemberCourse struct {
	...
	// userId
	UserId uint `json:"user_id" gorm:"column:user_id;type:bigint(32);comment:用户 id"`
	// 开始时间
	StartTime time.Time `json:"start_time" gorm:"column:start_time;type:timestamp;comment:开始时间"`
	// 结束时间
	EndTime time.Time `json:"end_time" gorm:"column:end_time;type:timestamp;comment:结束时间"`
	// 总课程数
